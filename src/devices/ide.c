...
void ide_init(void) {
  ...
  for (chan_no = 0; chan_no < CHANNEL_CNT; chan_no++) {
    ...
    for (dev_no = 0; dev_no < 2; dev_no++) {
      ...
      snprintf(d->name, sizeof d->name, "hd%c", (char)('a' + chan_no * 2 + dev_no));
      d->channel = c;
      d->dev_no = dev_no;
      d->is_ata = false;
    }
...
static void input_sector(struct channel *c, void *sector) { insw(reg_data(c), sector, BLOCK_SECTOR_SIZE / 2); }

/** Writes SECTOR to channel C's data register in PIO mode.
   SECTOR must contain BLOCK_SECTOR_SIZE bytes. */
static void output_sector(struct channel *c, const void *sector) { outsw(reg_data(c), sector, BLOCK_SECTOR_SIZE / 2); }
