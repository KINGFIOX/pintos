...
static bool archive_ordinary_file(const char *file_name, int file_fd, int archive_fd, bool *write_error) {
  ...
  if (!write_header(file_name, USTAR_REGULAR, file_size, archive_fd, write_error)) return false;
...
static bool archive_directory(char file_name[], size_t file_name_size, int file_fd, int archive_fd, bool *write_error) {
  ...
  if (!write_header(file_name, USTAR_DIRECTORY, 0, archive_fd, write_error)) return false;
...
static bool write_header(const char *file_name, enum ustar_type type, int size, int archive_fd, bool *write_error) {
