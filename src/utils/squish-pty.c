...
static void fail_io(const char *msg, ...) __attribute__((noreturn)) __attribute__((format(printf, 1, 2)));
...
static void relay(int pty, int dead_child_fd) {
  ...
  while (pipes[1].in != -1) {
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (p->in != -1 && p->size + p->ofs < sizeof p->buf) FD_SET(p->in, &read_fds);
...
int main(int argc __attribute__((unused)), char *argv[]) {
  ...
  */
  ...
  if (setitimer(ITIMER_VIRTUAL, &zero_itimerval, &old_itimerval) < 0) fail_io("setitimer");
  ...
  if (pid < 0)
    ...
    if (setitimer(ITIMER_VIRTUAL, &old_itimerval, NULL) < 0) fail_io("setitimer-child");
    if (dup2(slave, STDOUT_FILENO) < 0) fail_io("dup2");
    if (close(pipe_fds[0]) < 0 || close(pipe_fds[1]) < 0 || close(slave) < 0 || close(master) < 0) fail_io("close");
